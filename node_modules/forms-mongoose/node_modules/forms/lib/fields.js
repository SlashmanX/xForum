...
exports.string = function(opt){
    ...
    f.labelText = function(name){
        ...
        if(!label){
	// First get rid of 'permissions.' then capitalise each word, found here: http://stackoverflow.com/questions/4878756/javascript-how-to-capitalize-first-letter-of-each-word-like-a-2-word-city
            label = name.substr(name.indexOf('.')+1).replace(/_/g, ' ').replace(/\w\S*/g, function(txt){return txt.charAt(0).toUpperCase() + txt.substr(1).toLowerCase();});
        }
        return label;
    };
    f.labelHTML = function(name, id){
        return '<label class = "control-label" for="' + (id || 'id_'+name) + '">' +
